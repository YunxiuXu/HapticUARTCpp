...
int main()
{
    ...
    while (true) {
        ...
        int data_to_send = ((127 * (std::sin(2 * PI * frequency * elapsed_time) + 1.0)));// -14; // 计算正弦波的值
        data_to_send = (float)data_to_send / 255 * 300;
        //char numberStr[20];
...
double calculateSin(double amplitude, double frequency, double phaseShift, double t) {
    double radians = 2.0 * PI * frequency * t + phaseShift;
